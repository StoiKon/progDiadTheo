<?php
function setUsername($con,$id,$name){
	$stmt=$con->prepare("UPDATE `Usr` SET `name`=? WHERE id = ?");
...
function setEmail($con,$id,$email){
	$stmt=$con->prepare("UPDATE `Usr` SET email=? WHERE id = ?");
...
function setTFullname($con,$tid,$fname){
	$stmt=$con->prepare("UPDATE Teacher SET fullname= ? WHERE id = ?");
	if($stmt == false){
		print($con->error);
	}
    $stmt->bind_param("si",$fname,$tid);
	if($stmt == false){
		print($con->error);
	}
	$stmt->execute();
}
function setStFullname($con,$fname,$am){
	$stmt=$con->prepare("UPDATE `Student` SET `fullname`=? WHERE am =?");
	if($stmt == false){
		print($con->error);
	}
    $stmt->bind_param("ss",$fname,$am);
	$stmt->execute();
}
function setStEntYear($con,$ent,$am){
	$stmt=$con->prepare("UPDATE `Student` SET `EntYear`=? WHERE am =?");
	if($stmt == false){
		print($con->error);
	}
    $stmt->bind_param("is",$ent,$am);
	$stmt->execute();
}
function getUserById($con,$id){
  ...
  //SELECT fullname,a.* FROM Teacher ,(SELECT * FROM `Usr` WHERE 1) as a WHERE tId = Teacher.id
  function getTeachers($con){
	$sql="SELECT * FROM Teacher ,(SELECT * FROM `Usr` WHERE 1) as a WHERE tId = Teacher.id";
	$stmt=$con->prepare($sql);
	$stmt->execute();
	$result = $stmt->get_result();
	return $result;
...
function getStudents($con){
	$sql="SELECT * FROM Student,(SELECT * FROM `Usr` WHERE 1) as a WHERE stAm = Student.am ";
	$stmt=$con->prepare($sql);
	$stmt->execute();
	$result = $stmt->get_result();
	return $result;
}
function getTeacher($con,$tid){
...
function getStudent($con,$stAm){
	$sql="SELECT * FROM Student WHERE am =?";
	$stmt=$con->prepare($sql);
